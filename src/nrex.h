...
#include <vector>
#ifdef NREX_UNICODE
typedef wchar_t NRexChar;
#else
typedef char NRexChar;
#endif
...
public:
    ...
    * \see nrex::compile()
    */
   NRex(const NRexChar* pattern, int captures = 9);
    ...
    */
   bool Compile(const NRexChar* pattern, int captures = 9);
    ...
    */
   bool Match(const NRexChar* str, std::vector<NRexResult>& captures, int offset = 0, int end = -1) const;

   const std::string& GetErrorString() const { return m_errString; }

private:
    int m_capturing;
    unsigned int m_lookaheadDepth;
    NRexNode* m_root;
